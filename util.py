...
def rotate(args, image):
    transform_ro = transforms.Compose(
    ...
    image_ro = transform_ro(image)
    image_ro = image_ro.unsqueeze(0)
    image_ro.to(device)
    return image_ro
...
def bisectSearch(fitness):
    ...
    return bisect_left(accumulator, rndPoint)
...
def find_vgg_layer(arch, target_layer_name):
    ...
    if len(hierarchy) == 2:
        target_layer = target_layer.features[int(hierarchy[1])]
